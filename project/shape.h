...
typedef struct AbTri {
  void (*getBounds)(const struct AbTri *shape, const Vec2 *centerPos, Region *bounds);
  int (*check)(const struct AbTri *shape, const Vec2 *centerPos, const Vec2 *pixelLoc);
  int size;
} AbTri;
